...
def create_resume_metadata(list_of_pdf, dir_path):
    ...
    creation = [datetime.fromtimestamp(os.path.getmtime(dir_path+f)) for f in list_of_pdf if f.endswith(('.pdf','.docx'))]
    title = [f for f in list_of_pdf if f.endswith(('.pdf','docx'))]
...
def start():
    ...
    if os.path.exists(dir_path):
        ...
        new_resume_list = [f for f in os.listdir(dir_path) if f.endswith(('.pdf','.docx')) and f not in resume_metadata.Title.unique().tolist()]
        ...
        # Else loop for initial run
            ...
            if os.path.exists(f'./static/jd_skills/employee_skill_matrix.xlsx')==False:
                ...
                #try:
                skill_matrix = main(dir_path,System_Prompt,[f for f in os.listdir(dir_path) if f.endswith(('.pdf','.docx'))],selected_model)
